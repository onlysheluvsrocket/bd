...
## 2. ER-диаграмма
erDiagram
    СЧЕТА ||--o{ ТРАНЗАКЦИИ : "хранят"
    КАТЕГОРИИ ||--o{ ТРАНЗАКЦИИ : "классифицируют"
    КАТЕГОРИИ ||--o{ ПЛАНЫ_БЮДЖЕТА : "планируются"

    СЧЕТА {
        integer id PK "Суррогатный первичный ключ"
        varchar name "Наименование счета"
        varchar bank_name "Название банка"
        varchar type "Тип: дебетовый/кредитный/наличные"
        numeric balance "Текущий баланс"
    }

    КАТЕГОРИИ {
        integer id PK "Суррогатный первичный ключ"
        varchar name "Наименование категории"
        varchar cat_type "Тип операции: доход/расход"
    }

    ТРАНЗАКЦИИ {
        integer id PK "Суррогатный первичный ключ"
        integer account_id FK "Ссылка на счет"
        integer category_id FK "Ссылка на категорию"
        numeric amount "Сумма операции"
        date operation_date "Дата совершения"
        text description "Описание/комментарий"
    }

    ПЛАНЫ_БЮДЖЕТА {
        integer id PK "Суррогатный первичный ключ"
        integer category_id FK "Категория расхода"
        date month "Месяц планирования"
        numeric planned_amount "Плановая сумма"
    }
...
6. Примеры SQL-запросов для выходных документов
...
7. Пример заполнения таблиц тестовыми данными

8. -- Вставка тестовых данных (минимум 4 строки в каждой таблице)
